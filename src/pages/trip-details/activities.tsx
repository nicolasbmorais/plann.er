                ...
                return (
                    <div key={category.date} className="space-y-2.5">
                        ...
                        {category.activities.length > 0
                            ? (<div className="">
                                {category.activities.map(activities => {
                                    return (
                                        <div key={activities.id} className="px-4 py-2.5 bg-zinc-900 rounded-xl shadow-shape flex items-center gap-3">
                                            <CircleCheck className="size-5 text-lime-300" />
                                            <span className="text-zinc-100 ">{activities.title}</span>
                                            <span className="text-zinc-400 text-sm ml-auto">{format(new Date(activities.occurs_at), 'HH:mm', { locale: ptBR })}h</span>
                                        </div>
                                    )
                                })}
                            </div>)
